...
Standing environmental issue, not a code finding — **corrected 2026-08-06**. An earlier
version of this note said GitHub Actions had stopped dispatching repository-wide and
suggested checking Actions billing or enablement. That was wrong, and chasing it would have
wasted time. Actions is enabled and healthy; runs completed successfully as recently as
20:30 UTC today.

What is actually happening: **workflow runs are not created for events generated by the
GitHub App token this automation uses.** GitHub suppresses them to prevent recursive runs.
The evidence separates cleanly:

| Action | How it was done | Workflow run created? |
| --- | --- | --- |
| Push to a `claude/*` branch | `git push` from the container | Yes — Codex Review Bot ran |
| Open PRs #43–#45 | GitHub API, app token | No `pull_request` run |
| Merge PRs #42–#45 to `main` | GitHub API, app token | No `push` run |
| Merge at `6a5f939` (2026-08-04) | By the repository owner | Yes — CI *and* Desktop build ran |

So the last `ci.yml` and `desktop.yml` runs on `main` are both from `6a5f939`
(2026-08-04) — before the dangerous goods workflow and every fix since. Consequences worth
knowing:

- Merges landed by this automation are verified **locally only** (`npm run check`).
- **The packaged Windows installer is stale.** `desktop.yml` last produced one from
  `6a5f939`, so the newest `.exe` contains none of the dangerous goods workflow, the parser
  fixes, or the twelve findings above.

Neither needs a settings change. To refresh both, either run **Actions → Desktop build →
Run workflow** (it already declares `workflow_dispatch`), or land any commit on `main`
using the owner's own credentials. This automation cannot do the first — dispatching
returns `403 Resource not accessible by integration`, because the app token lacks
`actions: write`.
