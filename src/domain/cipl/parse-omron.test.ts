...
beforeAll(async () => {
  ...
  parsed.K78464FJ = await parseCipl('K78464FJ_CIPL.pdf', readFixture('K78464FJ'))
  parsed.K78027EC = await parseCipl('K78027EC_CIPL.pdf', readFixture('K78027EC'))
}, 60_000)
...
describe.skipIf(!hasFixtures())('G78495IQ — Nippon Express, 3 pieces, two duplicate cable lines', () => {
  ...
  it('distinguishes SOLD TO from CONSIGNED TO', () => {
    const h = parsed.G78495IQ.headers.FC
    expect(h.consignedTo.name).toBe('Omron Automation Pvt. Ltd. - Edi')
  ...
  it('collects every order number', () => {
    expect(parsed.G78495IQ.headers.FC.orderNumbers).toEqual([
      '00299378OP0080',
      '00300015OP0080',
      '00296253OP0080',
    ])
  ...
  it('reads all three invoice lines with values', () => {
    const lines = fcInvoice(parsed.G78495IQ)
  ...
  it('reads per-line packing weights from inside the parentheses, not the running totals', () => {
    const lines = fcPacking(parsed.G78495IQ)
    expect(lines).toHaveLength(3)
  ...
  it('prices TP1 in the destination currency, not USD', () => {
    const h = parsed.G78495IQ.headers.TP1
...
describe.skipIf(!hasFixtures())('K78464FJ — single line, consignee differs from buyer', () => {
  it('uses CONSIGNED TO for the receiving party', () => {
    ...
    expect(h.consignedTo.name).toBe('Omron Industrial Automation Ch')
    expect(h.dischargePort).toBe('Shanghai, China')
  })

  it('reads the single invoice line', () => {
    const lines = fcInvoice(parsed.K78464FJ)
...
describe.skipIf(!hasFixtures())('K78027EC — CEVA, 11 lines over 8 orders across page breaks', () => {
  ...
  it('keeps the four repeated 4208669164 sequences apart', () => {
    const repeated = fcInvoice(parsed.K78027EC).filter((l) => l.orderNumber === '4208669164')
  ...
  it('recovers the two order numbers the header P/O field truncates', () => {
    const header = parsed.K78027EC.headers.FC
    ...
    expect(parsed.K78027EC.warnings.join(' ')).toMatch(/header P\/O field lists 6/)
  })
  ...
  it('reads packing weights totalling the document total', () => {
    const lines = fcPacking(parsed.K78027EC)
    expect(lines).toHaveLength(11)
    const net = lines.reduce((s, l) => s + (l.netWeightKg ?? 0), 0)
    expect(net).toBeCloseTo(138.841, 2)
    expect(parsed.K78027EC.headers.FC.totalNetWeightKg).toBeCloseTo(138.841, 3)
    expect(parsed.K78027EC.headers.FC.totalGrossWeightKg).toBeCloseTo(171.911, 3)
  })

  it('prices TP1 in EUR', () => {
    expect(parsed.K78027EC.headers.TP1.documentCurrency).toBe('EUR')
    expect(parsed.K78027EC.headers.TP1.totalValue).toBeCloseTo(82126.18, 2)
  })
})
