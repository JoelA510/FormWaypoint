/**
 * Parser for the SAP-style "OMRON SHIPMENT#" commercial invoice and master packing list.
 *
...
export function isOmronShipmentFormat(pages: TextPage[]): boolean {
  const firstPage = pages[0]
  if (!firstPage) return false
  const text = firstPage.rows.map(rowText).join(' ')
  return /OMRON SHIPMENT#/i.test(text)
}

export function parseOmronShipmentPages(fileName: string, pages: TextPage[]): ParsedCipl {
...
export async function parseOmronShipment(fileName: string, data: ArrayBuffer | Uint8Array): Promise<ParsedCipl> {
  return parseOmronShipmentPages(fileName, await extractTextPages(data))
}
...
function parseHeader(page: TextPage): ShipmentHeader {
  ...
  const shipmentNumber = rightOfLabel(rows, 'OMRON SHIPMENT#') ?? ''
...
function endOfTable(rows: TextRow[], from: number): number {
  for (let i = from + 1; i < rows.length; i++) {
    ...
    if (text.includes('TOTAL NET VALUE') || text.includes('SUMMARY INFORMATION') || text.includes('OMRON ROBOTICS')) {
      return i
    }
  }
  return rows.length
...
function parseBlock(block: TextRow[], page: TextPage, kind: DocumentKind, currency: string): SourceLine | null {
  ...
  // dual-currency shipment repeats them underneath in the same columns (278514 prints USD
