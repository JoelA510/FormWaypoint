...
/**
 * Omron CIPLs contain the same shipment twice: an `FC` set priced in USD and a `TP1` set
...
export interface SourceLine {
   ...
   *
   * Which order depends on the format: `omron-fc-tp1` prints the *customer's* purchase
   * order here, while `omron-shipment` prints Omron's own sales order and carries the
...
export interface ParsedCipl {
   ...
   *
   * Only the `omron-shipment` layout publishes this — its master packing list ends with a
