...
describe.skipIf(!hasFixtures())('document set selection', () => {
  it('always chooses the USD set, never the later destination-currency one', () => {
    ...
    expect(parsed.K78027EC.headers.TP1.documentCurrency).toBe('EUR')
  })
...
describe.skipIf(!hasFixtures())('G78495IQ — merges two cable lines, keeps the motor separate', () => {
  it('produces the quantities, weights and values on the filed SLI', () => {
    const { sliLines } = reconcile(parsed.G78495IQ, scheduleB, CONTROLLED)
  ...
  it('does not adopt the classification change made on the historical SLI', () => {
    ...
    const overridden = reconcile(parsed.G78495IQ, scheduleB, {
      ...CONTROLLED,
  ...
  it('reconciles to the printed document totals', () => {
    const { checks, canGenerate } = reconcile(parsed.G78495IQ, scheduleB, CONTROLLED)
...
describe.skipIf(!hasFixtures())('K78464FJ — single line', () => {
  it('reproduces the filed row', () => {
    const { sliLines } = reconcile(parsed.K78464FJ, scheduleB, CONTROLLED)
  ...
  it('resolves the destination country from the discharge port', () => {
    expect(resolveDestinationCountry(parsed.K78464FJ.headers.FC)).toBe('China')
  })
})

describe.skipIf(!hasFixtures())('K78027EC — CEVA, groups across different part numbers', () => {
  it('reproduces all three filed rows exactly', () => {
    const { sliLines } = reconcile(parsed.K78027EC, scheduleB, CONTROLLED)
  ...
  it('reconciles to the printed totals', () => {
    const { checks, canGenerate } = reconcile(parsed.K78027EC, scheduleB, CONTROLLED)
  ...
  it('resolves the destination country the consignee address never states', () => {
    ...
    expect(resolveDestinationCountry(parsed.K78027EC.headers.FC)).toBe('Netherlands')
  })
})

describe.skipIf(!hasFixtures())('Schedule B validation', () => {
  it('accepts the codes actually used and rejects a retired one', () => {
    const { checks } = reconcile(parsed.K78027EC, scheduleB, CONTROLLED)
  ...
  it('does not block generation on advisory findings', () => {
    const { canGenerate } = reconcile(parsed.K78027EC, scheduleB, CONTROLLED)
    expect(canGenerate).toBe(true)
  })
})
...
describe.skipIf(!hasFixtures())('form capacity', () => {
  it('blocks when a shipment needs more rows than the form holds', () => {
    const tight = reconcile(parsed.K78027EC, scheduleB, { ...CONTROLLED, maxRows: 2 })
    const capacity = tight.checks.find((c) => c.id === 'row-capacity')
    expect(capacity?.passed).toBe(false)
    expect(tight.canGenerate).toBe(false)

    const roomy = reconcile(parsed.K78027EC, scheduleB, { ...CONTROLLED, maxRows: 8 })
...
describe.skipIf(!hasFixtures())('classification applicability', () => {
  it('accepts a correct code even when the group heading is worded differently', () => {
    ...
    const { checks } = reconcile(parsed.G78495IQ, scheduleB, CONTROLLED)
    const motorRow = reconcile(parsed.G78495IQ, scheduleB, CONTROLLED).sliLines.findIndex(
  ...
  it('still rejects a code that does not describe the goods', () => {
    const { checks } = reconcile(parsed.G78495IQ, scheduleB, {
      ...CONTROLLED,
      overrides: { '8544420000': '8483.10.5000' },
    })
