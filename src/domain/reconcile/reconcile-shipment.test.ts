...
describe.skipIf(!hasFixtures())('sales orders and customer purchase orders stay apart', () => {
  it('files the customer PO in the consignee-PO box, not vendor\'s sales order', () => {
    ...
    // vendorB2's single line: SO 13393035, customer PO 1413823.
    expect(parsed['vendorB2'].headers.FC.orderNumbers).toEqual(['13393035'])
    expect(parsed['vendorB2'].headers.FC.purchaseOrders).toEqual(['1413823'])
  })
...
describe.skipIf(!hasFixtures())('shipment reference', () => {
  ...
  it('prefills from the sales orders on a layout that carries them', () => {
    const result = reconcile(parsed['vendorB1'], scheduleB, {
      ...CONTROLLED,
      unitWeightsByPart: UNIT_WEIGHTS_vendorB1,
      maxRows: 8,
    })
    const draft = buildDraft(result, PROFILE, defaultShipmentSettings(nippon), nippon)
    // Matches the filed vendorB1 SLI box 11 exactly: three shown, three abbreviated.
