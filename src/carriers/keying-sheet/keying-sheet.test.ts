 ...
 *
 * The reference is shipment K78541WH as it was actually keyed: seven commodities, weights
...
function fixture(lines: MergedLine[], rows: SLILine[]): Reconciliation {
  return {
    selectedSet: 'FC',
    header: {
      invoiceNumber: 'K78541WH',
      invoiceDate: 'July 28, 2026',
      onAboutDate: null,
      soldTo: { name: 'Omron Corporation', lines: [], country: null },
      consignedTo: {
        name: 'Omron Asia Pacific Pte. Ltd.',
...
describe('FedEx Ship Manager sheet', () => {
  ...
  it('carries the invoice and purchase order into Additional references', () => {
    ...
    expect(fields.find((f) => f.label === 'Invoice number')!.value).toBe('K78541WH')
...
describe('UPS WorldShip sheet', () => {
  ...
  it('puts the invoice number in Reference Number 1', () => {
    ...
    expect(refs[0]).toMatchObject({ label: 'Reference Number 1', value: 'K78541WH' })
  })
})
