...
function parseHeader(textBlock) {
  ...
  for (let i = 0; i < relevantHeaderLines.length; i++) {
    ...
    // --- SHIPPER (Top Left) ---
    // Usually starts with "vendor" and is NOT "Sold To" or "Consigned To"
    // It ends when we hit "SOLD TO" or "DATE" or other labels
    if (i < 10 && line.includes('vendor') && !lower.includes('sold to') && !lower.includes('consigned to') && !header.shipper) {
      header.shipper = line;
