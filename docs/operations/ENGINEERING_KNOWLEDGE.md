...
- **Date**: 2026-07-30
- **Context & Problem**: One Vendor A shipment printed three things no other document did —
  a line block split by a page break, weights printed divided (`(@ / 6)`), and a commodity
...
- **Solution & Pattern**: The blocking checks did their job — nothing wrong was ever
  filed — but a blocked shipment is still a shipment nobody can file. That document became
  a fixture with its own suite (`src/domain/cipl/parse-vendor-a-pagebreak.test.ts`).
