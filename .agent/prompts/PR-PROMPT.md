...
- **Second CIPL format:** The Vendor B `SHIPMENT#` layout now sits behind a format registry that detects the document and dispatches to its own parser; everything downstream is unchanged, so a third format is a detector and a parser.
